...
class GenerationMixin:
    ...
    def _prepare_attention_mask_for_generation(
        ...
        is_pad_token_in_inputs = (pad_token_id is not None) and (
            isin_mps_friendly(
                elements=inputs, test_elements=pad_token_id).any()
        )
        is_pad_token_not_equal_to_eos_token_id = (eos_token_id is None) or ~(
            isin_mps_friendly(elements=eos_token_id,
                              test_elements=pad_token_id).any()
        )
        can_infer_attention_mask = is_pad_token_in_inputs * \
    ...
    def _get_logits_processor(
        ...
        if generation_config.do_sample:
            ...
            if generation_config.min_p is not None:
                ...
                processors.append(
                    MinPLogitsWarper(min_p=generation_config.min_p,
                                     min_tokens_to_keep=min_tokens_to_keep)
                )
    ...
    def _validate_assistant(self, assistant_model):
        ...
        if self.config.is_encoder_decoder and not assistant_model.config.is_encoder_decoder:
            attributes_to_check = [
                "encoder_attention_heads", "encoder_ffn_dim", "encoder_layers"]
            attributes_to_check = [attr for attr in dir(
                assistant_model.config) if attr in attributes_to_check]
            are_equal = all(
    ...
    def _get_initial_cache_position(self, input_ids, model_kwargs):
        ...
        if "inputs_embeds" in model_kwargs:
            cache_position = torch.ones_like(
                model_kwargs["inputs_embeds"][0, :, 0], dtype=torch.int64).cumsum(0) - 1
        else:
            cache_position = torch.ones_like(
                input_ids[0, :], dtype=torch.int64).cumsum(0) - 1
    ...
    def _get_cache(
        ...
        if need_new_cache:
            ...
            if hasattr(self, "hf_device_map"):
                main_device = [d for d in self.hf_device_map.values() if d not in [
                    "cpu", "disk"]][0]
    ...
    def _prepare_cache_for_generation(
        ...
        cache_name = "past_key_values" if "mamba" not in self.__class__.__name__.lower(
        ) else "cache_params"
        requires_cross_attention_cache = (
            self.config.is_encoder_decoder or model_kwargs.get(
                "encoder_outputs") is not None
    ...
    def _prepare_special_tokens(
        ...
        if not is_torchdynamo_compiling():  # Checks that depend on tensor-dependent control flow
            ...
            if eos_token_tensor is not None and (
                torch.is_floating_point(eos_token_tensor) or (
                    eos_token_tensor < 0).any()
            ):
                logger.warning(
    ...
    def generate(
        ...
        **kwargs,
        ...
        if (
            # Should check for `True` after v4.47
    ...
    def heal_tokens(
        ...
        prompts = [p.strip() for p in tokenizer.batch_decode(
        ...
        input_ids = torch.where(
            input_ids == bos_token_id, pad_token_id, input_ids)
        ...
        tail_toks = (tokenizer.decode(t).replace(" ", space_tok)
                     for t in tail_ids)
    ...
    def _dola_decoding(
        ...
        **model_kwargs,
        ...
        if self.config.is_encoder_decoder:
            raise ValueError(
                "DoLa decoding is only available for decoder-only models.")
        # init values
        ...
        # Set the `dola_layers` to a list of integers for layer indices to contrast manually specified layers.
            ...
            candidate_premature_layers = [
                i for i in dola_layers if i < final_layer]
        else:
            raise ValueError(
        ...
        while self._has_unfinished_sequences(this_peer_finished, synced_gpus, device=input_ids.device):
            ...
            final_layer_next_token_logits = outputs.logits[:, -1, :].detach(
            ).clone().float()
        ...
        **model_kwargs,
        ...
        while self._has_unfinished_sequences(this_peer_finished, synced_gpus, device=input_ids.device):
            ...
            if model_kwargs.get("past_key_values") is None or (
                isinstance(model_kwargs["past_key_values"],
            ...
            if not sequential:
                ...
                if isinstance(past, DynamicCache) or (
                    isinstance(past, EncoderDecoderCache) and isinstance(
                        past.self_attention_cache, DynamicCache)
                ):
            ...
            if sequential:
                ...
                for i in range(top_k):
                    ...
                    if isinstance(outputs["past_key_values"], DynamicCache) or (
                        isinstance(outputs["past_key_values"],
                                   EncoderDecoderCache)
        ...
        **model_kwargs,
        ...
        while self._has_unfinished_sequences(this_peer_finished, synced_gpus, device=input_ids.device):
            ...
            if do_sample:
                probs = nn.functional.softmax(next_token_scores, dim=-1)
                next_tokens = torch.multinomial(
                    probs, num_samples=n_tokens_to_keep)
            ...
            if "cache_position" in candidate_kwargs:
                candidate_kwargs["cache_position"] = torch.cat(
                    (
                        ...
                        torch.arange(cur_len, cur_len + candidate_length,
...
def _ranking_fast(
    ...
    cosine_matrix_mask = (1 - cosine_matrix_mask) * \
...
def _split(data, full_batch_size: int, num_hidden_layers: int, split_size: int = None):
    ...
    # New cache format
        ...
        isinstance(data, EncoderDecoderCache) and isinstance(
            data.self_attention_cache, DynamicCache)
    ):
...
def _split_model_inputs(
    ...
    keys_to_ignore = ["cache_position",
                      "encoder_outputs", "num_logits_to_keep"]
    ...
    data_split_list = [
        {k: _split(model_input[k], full_batch_size, num_hidden_layers, split_size)[
            i] for k in non_bool_keys}
...
def _relative_top_filter(
    ...
    baseline_scores_normalized[scores_normalized <
                               probs_thresh] = base_filter_value
...
def _dola_select_contrast(
    ...
    if len(candidate_premature_layers) == 1:
        ...
        final_logits, base_logits = _relative_top_filter(
    ...
    kl1 = F.kl_div(
        log_softmax_mature_layer[None, :, :], avg_dist, reduction="none").mean(-1)
    # shape: (num_premature_layers, batch_size)
    kl2 = F.kl_div(log_softmax_premature_layers,
                   avg_dist, reduction="none").mean(-1)
