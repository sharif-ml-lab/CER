...
def evaluate_batch_examples(
    ...
    for i, question in enumerate(batch_questions):
        ...
        try:
            ...
            correct_answer = float(postprocess_final_answer(correct_answer_str))
            is_correct = abs(model_answer - correct_answer) <= 1e-2
        except ValueError:
            print(f'Can not compare correct {correct_answer_str} with predicted {predicted_final_answer}')
            is_correct = False
