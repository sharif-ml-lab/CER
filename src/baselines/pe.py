...
def get_pe(normalized_length, generated_logits, generated_ids, pad_token_id):
    probabilities = F.softmax(generated_logits, dim=-1)
    entropy = -torch.sum(probabilities *
                         torch.log(probabilities + 1e-9), dim=-1)
...
def _k_generation(
    ...
    for _ in range(k):
        ...
        for i in range(batch_size):
            ...
            pe = get_pe(normalize_length, output_scores, answer_ids,
                        pad_token_id=tokenizer.pad_token_id)
